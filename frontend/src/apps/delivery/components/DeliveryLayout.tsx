import { Home, Package, User, IndianRupee } from "lucide-react";
    ...
    <div className="min-h-screen flex flex-col bg-[var(--color-background)]">
      ...
      <header className="sticky top-0 z-30 bg-white/90 backdrop-blur-md border-b border-gray-100 px-4 py-3 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-base font-bold text-gray-900">VillageMart <span className="text-green-600">Delivery</span></h1>
        </div>
      ...
      <nav className="sticky bottom-0 z-30 bg-white/95 backdrop-blur-md border-t border-gray-100 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
        <div className="flex items-center justify-around py-2">
          {tabs.map((tab) => {
            const isActive = subPath === tab.to || (subPath === "" && tab.to === "");
            return (
              <button key={tab.to} onClick={() => navigateToQueryPath("delivery", tab.to)}
                className={`flex flex-col items-center gap-0.5 px-4 py-1.5 rounded-xl transition-all duration-200 min-w-[64px] ${
              ...
              >
                ...
                <span className={`text-[10px] font-semibold ${isActive ? "text-green-700" : "text-gray-400"}`}>{tab.label}</span>
              </button>
            );
          })}
        </div>

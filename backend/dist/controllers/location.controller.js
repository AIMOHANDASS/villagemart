...
const safeAddColumn = (table, column, definition) => {
    db_1.default.query(`SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME=? AND COLUMN_NAME=?`, [table, column], (err, rows) => {
        if (!err && (!rows || rows.length === 0)) {
            db_1.default.query(`ALTER TABLE ${table} ADD COLUMN \`${column}\` ${definition}`, (addErr) => {
                if (addErr) {
...
const getNearbyDrivers = (req, res) => {
    ...
    ORDER BY distance;
  `;
    db_1.default.query(sql, [lat, lng, lat, type], (err, rows) => {
        if (err) {
...
exports.default = { getNearbyDrivers: exports.getNearbyDrivers };

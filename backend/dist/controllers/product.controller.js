...
const createProduct = (req, res) => {
    ...
    const { E_name, T_name, MRP, s_price, GST, category, product_type, inStock, isOrganic, } = req.body;
    ...
    db_1.default.getConnection((err, conn) => {
      ...
      `;
            ...
            conn.query(insertSql, params, (insertErr, result) => {
                if (insertErr) {
                    ...
                    if (imageInsertErr) {
                        return conn.rollback(() => {
                            ...
                            return res.status(500).json({
                                ...
                                error: imageInsertErr.sqlMessage || imageInsertErr.message,
                            });
                        });
                    }
...
const updateProduct = (req, res) => {
    ...
    for (const key of Object.keys(body)) {
        if (UPDATABLE_COLUMNS.has(key)) {
            ...
            if (key === "imageurl" && primaryImageUrl !== null)
                continue;
            setClauses.push(`\`${key}\` = ?`);
        ...
        conn.beginTransaction((txErr) => {
          ...
          `;
            ...
            if (setClauses.length > 0) {
                ...
                conn.query(updateSql, values, (updateErr, result) => {
                    if (updateErr) {
                        return conn.rollback(() => {
                            conn.release();
